...
export default function CreateThemeModal({ isOpen, onClose }: CreateThemeModalProps) {
  const { createThemeStep, setCreateThemeStep, createThemeData, setCreateThemeData, themeList, setThemeList, setTheme, setCurrentMusic, setIsPlaying } = useTheme()
  ...
  const removeTag = (tagToRemove: string) => {
    updateCreateThemeData('tags', createThemeData.tags?.filter(tag => tag !== tagToRemove))
  }
  ...
  const CreateNewTheme = () => {
    if (themeList.find(theme => theme.name === createThemeData.name)) {
     ...
     setThemeList([...themeList, {...createThemeData, id: crypto.randomUUID()}])
     toast({
       title: "Success",
  ...
  const updateTheme = () => {
    ...
    if (!theme) return
    setThemeList(
        themeList.map(t => t.id === createThemeData.id ? {...createThemeData} : t)
    )
    toast({
        title: "Success",
        description: "Theme updated successfully"
      })
      setTheme(createThemeData.colorScheme.split(" ")[1] as "dark" | "light" | "system" | "lofi" | "nature" | "rain" | "ocean" | "forest" | "cafe" | "warm" | "cool" | "neutral" | "vibrant" | "pastel" | "monochrome" | "sunset" | "midnight" | "trees" | "desert" | "aurora" | "neon")
      setCurrentMusic({
  ...
  const uploadAudioHandler = async (file: File) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('Cloudinary cloud name is not configured')) {
        ...
        } else if (error.message.includes('Cloudinary upload preset is not configured')) {
          errorMessage = "Cloudinary upload preset is not configured. Please check your environment variables."
          ...
          errorMessage = "Upload preset permissions denied. Please check your Cloudinary settings."
        } else {
          errorMessage = error.message
        }
      }
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-foreground">Create New Theme Experience</h3>
              <motion.button
                onClick={handleClose}
            ...
            {/* Step Progress */}
                   ...
                   <div className="space-y-4">
                     <div>
                       ...
                       <input
                         ...
                         onChange={(e) => updateCreateThemeData('name', e.target.value)}
                     ...
                     <div>
                       ...
                       <textarea
                         ...
                         onChange={(e) => updateCreateThemeData('description', e.target.value)}
                     ...
                     <div>
                       ...
                       <div className="grid grid-cols-3 gap-3">
                         {[
                          ...
                          <motion.button
                            key={scheme.name}
                            onClick={() => updateCreateThemeData('colorScheme', `${scheme.emoji} ${scheme.name}`)}
                            className={`p-3 rounded-lg border transition-all duration-200 ${
                              createThemeData.colorScheme === `${scheme.emoji} ${scheme.name}`
                          ...
                          >
                            <div className="flex flex-col items-center space-y-1">
                              ...
                              <span className="text-xs capitalize">{scheme.desc}</span>
                            </div>
                          </motion.button>
                        ))}
                      </div>
                 ...
                 {createThemeStep === 3 && (
                   <div className="space-y-4">
                     <div>
                       <label className="block text-sm font-medium text-foreground mb-2">
                         Audio File
                                                                          ...
                                                                          { !createThemeData.src ? <div 
                             className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary/50 transition-colors cursor-pointer"
                         >
                             <input type="file" accept="audio/*" onChange={(e) => handleAudioFileChange(e)} className="hidden" id="audio-file-input" />
                        ...
                        <div className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary/50 transition-colors cursor-pointer relative">
                            <button className="absolute top-2 right-2 z-10 p-1 rounded-full bg-background/80 hover:bg-background" onClick={() => {
                            ...
                            <AudioWaveform className="w-8 h-8 mx-auto text-muted-foreground" />
                            <p className="text-sm text-muted-foreground">
                               {createThemeData.colorScheme.split(" ")[0]} {createThemeData.name}
                            </p>
                     ...
                     <div className="bg-muted/50 rounded-lg p-4">
                       <h4 className="font-medium text-foreground mb-2">Theme Summary</h4>
                       <div className="space-y-2 text-sm">
                         <div><span className="text-muted-foreground">Name:</span> {createThemeData.name || 'Not set'}</div>
                         <div><span className="text-muted-foreground">Color Scheme:</span> {createThemeData.colorScheme || 'Not set'}</div>
                         <div><span className="text-muted-foreground">Tags:</span> {createThemeData.tags?.length! > 0 ? createThemeData.tags?.join(', ') : 'None'}</div>
                       </div>
                     </div>
                   </div>
                 )}
             ...
             <div className="flex space-x-3 pt-4">
               ...
               {createThemeStep < 3 ? (
                 ...
                 <motion.button
                   disabled={!createThemeData.name || !createThemeData.colorScheme }
                   onClick={handleTheme}
                   className={`flex-1 p-3 rounded-lg bg-primary text-white hover:bg-primary/90 transition-colors dark:text-black ${!createThemeData.name || !createThemeData.colorScheme ? "opacity-50 cursor-not-allowed" : ""}`}
                   whileHover={{ scale: 1.02 }}
                   whileTap={{ scale: 0.98 }}
                 >
                       ...
                       <div className="flex items-center justify-between mb-4">
                         {[1, 2, 3].map((step) => (
                           <div key={step} className="flex items-center">
                             ...
                             {step < 3 && (
                               <div className={`w-16 h-1 mx-1 transition-all duration-300 ${
                                 step < createThemeStep ? 'bg-primary' : 'bg-muted'
                               }`} />
                             )}
                           </div>
                             ...
                             <div>
                               ...
                               <div className="grid grid-cols-3 gap-2">
                                 {[
                                   ...
                                   { name: 'vibrant', emoji: '🌈', desc: 'Vibrant' },
                                   { name: 'pastel', emoji: '🌸', desc: 'Pastel' },
                                   { name: 'monochrome', emoji: '⚫', desc: 'Mono' },
                                   { name: 'sunset', emoji: '🌅', desc: 'Sunset' },
                                   { name: 'midnight', emoji: '🌙', desc: 'Midnight' },
                         ...
                         {createThemeStep === 3 && (
                           <div className="space-y-4">
                             <div>
                               ...
                               >
                                 <input type="file" accept="audio/*" onChange={(e) => handleAudioFileChange(e)} className="hidden" id="mobile-audio-file-input" />
                                 <div className="space-y-2">
                                   <AudioWaveform className="w-6 h-6 mx-auto text-muted-foreground" />
                                   <p className="text-xs text-muted-foreground">
                                     Click to upload audio file or drag and drop
                                   </p>
                                   <p className="text-xs text-muted-foreground">
                             ...
                             <div className="bg-muted/50 rounded-lg p-3">
                               <h4 className="font-medium text-foreground mb-2 text-sm">Theme Summary</h4>
